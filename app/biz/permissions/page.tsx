...
export default function BizPermissionsPage() {
  ...
  const accentColor = '#0D9488'  // teal for biz

  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>

      {/* Sidebar */}
      <div style={{ width: 280, flexShrink: 0, background: '#fff', borderRight: '1px solid #E5E7EB', display: 'flex', flexDirection: 'column' }}>
        <div style={{ background: '#0D9488', color: '#fff', padding: '16px 20px', fontWeight: 800, fontSize: 13, letterSpacing: 1 }}>
          CORPORATE ROLES
        </div>
        <button
          onClick={() => setView(v => v === 'overrides' ? 'roles' : 'overrides')}
          style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '14px 20px', background: view === 'overrides' ? '#F0FDF4' : '#fff', border: 'none', borderBottom: '1px solid #E5E7EB', cursor: 'pointer', width: '100%', textAlign: 'left' }}
        >
          <span>🛡</span>
          <span style={{ fontSize: 13, fontWeight: 700, color: view === 'overrides' ? '#0D9488' : '#374151', textTransform: 'uppercase', letterSpacing: 0.5 }}>Member Overrides</span>
        </button>
        <div style={{ padding: '12px 20px 6px', fontSize: 11, fontWeight: 700, color: '#9CA3AF', letterSpacing: 1, textTransform: 'uppercase' }}>Corporate Roles</div>
          ...
          {loading ? <div style={{ padding: 20, color: '#9CA3AF', fontSize: 13 }}>Loading…</div>
            : roles.map(role => (
              <button key={role.id} onClick={() => { setActiveRoleId(role.id); setLocalMatrix(role.matrix ?? {}); setView('roles') }}
                style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', padding: '13px 20px', border: 'none', background: activeRoleId === role.id && view === 'roles' ? '#F0FDFA' : '#fff', color: activeRoleId === role.id && view === 'roles' ? accentColor : '#374151', fontWeight: activeRoleId === role.id ? 800 : 600, fontSize: 13, textAlign: 'left', cursor: 'pointer', borderLeft: activeRoleId === role.id && view === 'roles' ? `3px solid ${accentColor}` : '3px solid transparent' }}>
                <span style={{ textTransform: 'uppercase', letterSpacing: 0.3 }}>{role.label}</span>
                {activeRoleId === role.id && view === 'roles' && <span style={{ width: 8, height: 8, borderRadius: '50%', background: accentColor }} />}
              </button>
            ))}
        </div>
      </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <h1 style={{ fontSize: 20, fontWeight: 900, color: '#111827' }}>
                  ROLE SCHEMA: <span style={{ color: accentColor }}>{activeRole.label.toUpperCase()}</span>
                </h1>
              </div>
              <button onClick={commit} disabled={committing} style={{ padding: '12px 28px', borderRadius: 100, border: 'none', background: committed ? '#16A34A' : accentColor, color: '#fff', fontWeight: 800, fontSize: 13, cursor: 'pointer', letterSpacing: 0.5 }}>
